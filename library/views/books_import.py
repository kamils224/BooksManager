...
class BooksImport(View):
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        if request.POST['action'] == 'browse':
            context = {'browse_api_form': BooksImportApiForm(request.POST),
                       'books': self._get_api_objects(self._query_from_browse_api_form(request))}
            return render(request, 'books_import/index.html', context)

        elif request.POST['action'] == 'import':
            api_objects = self._get_api_objects(self._query_from_browse_api_form(request))
    ...
    def _query_from_browse_api_form(self, request: HttpRequest) -> str:
    ...
    def _get_api_objects(self, query: str) -> List[BookDto]:
        results = []
        try:
            response = requests.get(self.BASE_URL, params=query).json()
